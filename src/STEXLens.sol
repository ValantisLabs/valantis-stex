...
contract STEXLens {
        ...
        )
    {
        ...
        IWithdrawalModule withdrawalModule = IWithdrawalModule(
            stexInterface.withdrawalModule()
        );

        (reserve0Pool, reserve1Pool) = ISovereignPool(stexInterface.pool())
            .getReserves();
        ...
        amount1PendingLPWithdrawal = withdrawalModule
            .amountToken1PendingLPWithdrawal();
    }

    function getSharesForDeposit(
        address stex,
        uint256 amount
        ...
        (uint256 reserve0Pool, uint256 reserve1Pool) = ISovereignPool(
            stexInterface.pool()
        ).getReserves();
        ...
        uint256 reserve1PendingWithdrawal = withdrawalModule
            .amountToken1PendingLPWithdrawal();
        ...
        uint256 shares = Math.mulDiv(
            ...
            totalSupplyCache,
            reserve1Pool +
                withdrawalModule.amountToken1LendingPool() +
    ...
    function getAmountsForWithdraw(
        address stex,
        uint256 shares,
        ...
        (cache.reserve0Pool, cache.reserve1Pool) = ISovereignPool(
            stexInterface.pool()
        ).getReserves();
        ...
        cache.amount1LendingPool = Math.mulDiv(
            withdrawalModule.amountToken1LendingPool(),
            shares,
            totalSupplyCache
        );
        ...
        amount1 =
            ...
            Math.mulDiv(cache.reserve1Pool, shares, totalSupplyCache);

        if (isInstantWithdrawal) {
            uint256 amount1SwapEquivalent = stexInterface.getAmountOut(
                stexInterface.token0(),
                amount0,
                true
            );
            // Apply manager fee on instant withdrawals in token1
            cache.instantWithdrawalFee1 =
                (amount1SwapEquivalent *
                    ISovereignPool(stexInterface.pool()).poolManagerFeeBips()) /
                BIPS;
    ...
    function canClaim(
        address stex,
        uint256 idLPWithdrawal
    ) external view returns (bool) {
        IWithdrawalModule withdrawalModule = IWithdrawalModule(
            ISTEXAMM(stex).withdrawalModule()
        );
        ...
        if (
            ...
            request.cumulativeAmountToken1LPWithdrawalCheckpoint +
                request.amountToken1
        ) {
            return false;
        }
